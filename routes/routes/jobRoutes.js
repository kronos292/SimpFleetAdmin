...
router.get('/', async (req, res) => {
    ...
    if (req.query.searchBarQuery) {
        ...
        for (let i = 0; i < jobs.length; i++) {
            const job = jobs[i];
            if (job.vessel.vesselName.toLowerCase().includes(searchBarQuery) || job.vessel.vesselIMOID.toLowerCase().includes(searchBarQuery) || job.vessel.vesselCallsign.toLowerCase().includes(searchBarQuery) || (req.session.user && job.user._id === req.session.user._id && job.jobId.toLowerCase().includes(searchBarQuery))) {
    ...
    if (req.query.user_only === 'true') {
        const careOffParties = await CareOffParty.find({user: req.session.user._id}).populate({
            path: 'job',
            model: 'jobs',
            populate: [
                {
    ...
    const numLimit = req.query.numLimit && req.query.numLimit !== 'false' ? parseInt(req.query.numLimit) : jobs.length;
    const filteredJobs = [];
    for (let i = 0; i < jobs.length; i++) {
        if (i < numLimit) {
...
router.get('/index', async (req, res) => {
    const job = await Job.findOne({index: req.query.index}).populate({
        path: 'vessel',
        model: 'vessels'
    }).populate({
        path: 'user',
...
router.post('/', async (req, res) => {
    ...
    let {
        jobId, vesselIMOID, vesselName, vesselCallsign,
        vesselLoadingLocation, vesselArrivalDateTime, jobItems,
        jobOfflandItems, careOffParties, remarks, psaBerthingDateTime,
        psaUnberthingDateTime, vesselLoadingDateTime, createDSA, vesselLighterName,
        vesselLighterLocation, vesselLighterRemarks, otherVesselLocation
    } = req.body;
    ...
    if (vesselLoadingLocation === 'PSA') {
        let psaVessels = await PSAVessel.find({vesselIMO: vesselIMOID, actualUnberthTime: null}).select();
        if (psaVessels.length > 0) {
            psaVessels = psaVessels.sort((a, b) => {
                const berthTimeB = b.estimatedBerthTime !== null ? b.estimatedBerthTime : b.requiredBerthTime;
                const berthTimeA = a.estimatedBerthTime !== null ? a.estimatedBerthTime : a.requiredBerthTime;
                return berthTimeB - berthTimeA;
            });
            const latestPSAVessel = psaVessels[0];
            psaBerthingDateTime = latestPSAVessel.estimatedBerthTime !== null ? latestPSAVessel.estimatedBerthTime : latestPSAVessel.requiredBerthTime;
            psaUnberthingDateTime = latestPSAVessel.estimatedUnberthTime !== null ? latestPSAVessel.estimatedUnberthTime : latestPSAVessel.requiredUnberthTime;
    ...
    const jobTracker = new JobTracker({
        ...
        description: 'We have received your job booking and are currently checking the details. We will send you a confirmation email once everything is verified.',
        remarks: ''
    });
    ...
    const job = new Job({
        ...
        vessel,
        vesselLoadingLocation: vesselLoadingLocation === 'Others'? otherVesselLocation: vesselLoadingLocation,
    ...
    for (let i = 0; i < jobItems.length; i++) {
        ...
        const jobItemPricing = await JobItemPricing.findOne({uom: jobItemObj.uom}).select();
...
router.put('/', async (req, res) => {
    ...
    for (let i = 0; i < jobOfflandItems.length; i++) {
        ...
        if (jobOfflandItemObj === null) {
            ...
            await JobOfflandItem.findByIdAndUpdate(jobOfflandItem._id, jobOfflandItemObj);
        }
        jobOfflandItemObjs.push(jobOfflandItemObj);
    }
